...
describe("LaunchOrchestrator", () => {
	...
	it("looks up the active record + ledger grants + manifest, then calls launcher.launch", async () => {
		...
		// Scopes MUST be reconstructed onto the verb — dropping them (as this
		// path once did) silently broke every renderer that reasons over scoped
		// caps, e.g. the Agent's tool-offering (`intents.dispatch:propose-note`
		// never matched a bare `intents.dispatch`).
