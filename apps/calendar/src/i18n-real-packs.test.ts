...
import { LOCALE_PACK_IMPORTERS, MANIFEST } from "./i18n/t";
...
function firstDivergentKey(pack: Record<string, string>): string | undefined {
	return Object.keys(pack).find(
		(k) => k in MANIFEST && pack[k] !== (MANIFEST as Record<string, string>)[k],
	);
}

describe("calendar real overlay packs (12.15)", () => {
	for (const locale of OVERLAY_LOCALES) {
		it(`loads and overlays the ${locale} pack`, async () => {
			...
			const translate = createT(MANIFEST, pack ?? {});
			expect(translate(k as keyof typeof MANIFEST)).toBe(packRec[k]);
			expect(translate(k as keyof typeof MANIFEST)).not.toBe((MANIFEST as Record<string, string>)[k]);
		});
	}
});
