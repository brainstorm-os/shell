...
describe("mailbox locale packs (12.15)", () => {
	...
	const importers = {
		es: async () => ({
			default: { "app.title": "ES:Mailbox", "header.sidebar.show": "ES:Show sidebar" },
		}),
	};
	...
	it("applies the Spanish overlay once the pack resolves", async () => {
		...
		let out = "";
		function Probe() {
			out = useLocalePackT(MAILBOX_I18N, importers, rt)("app.title");
			return null;
		}
		act(() => root.render(<Probe />));
		await flush();
		expect(out).toBe("ES:Mailbox");
	});
	...
	it("re-derives when the runtime locale changes", async () => {
		...
		let out = "";
		function Probe() {
			out = useLocalePackT(MAILBOX_I18N, importers, rt)("header.sidebar.show");
			return null;
		}
		act(() => root.render(<Probe />));
		...
		await flush();
		expect(out).toBe("ES:Show sidebar");
	});
});
