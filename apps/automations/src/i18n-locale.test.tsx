...
import { AUTOMATIONS_I18N } from "./i18n";
...
describe("automations locale packs (12.15)", () => {
	...
	const importers = {
		es: async () => ({
			default: { "app.title": "ES:Automations", "header.moreActions": "ES:More actions" },
		}),
	};
	...
	it("applies the Spanish overlay once the pack resolves", async () => {
		...
		let out = "";
		function Probe() {
			out = useLocalePackT(AUTOMATIONS_I18N, importers, rt)("app.title");
			return null;
		}
		act(() => root.render(<Probe />));
		await flush();
		expect(out).toBe("ES:Automations");
	});
	...
	it("re-derives when the runtime locale changes", async () => {
		...
		let out = "";
		function Probe() {
			out = useLocalePackT(AUTOMATIONS_I18N, importers, rt)("header.moreActions");
			return null;
		}
		act(() => root.render(<Probe />));
