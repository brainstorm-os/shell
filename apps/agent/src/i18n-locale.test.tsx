...
describe("agent locale packs (12.15)", () => {
	...
	const importers = {
		es: async () => ({
			default: { "app.title": "ES:Agent", "header.newChat": "ES:New chat" },
		}),
	};
	...
	it("applies the Spanish overlay once the pack resolves", async () => {
		...
		let out = "";
		function Probe() {
			out = useLocalePackT(AGENT_I18N, importers, rt)("app.title");
			return null;
		}
		act(() => root.render(<Probe />));
		await flush();
		expect(out).toBe("ES:Agent");
	});
	...
	it("re-derives when the runtime locale changes", async () => {
		...
		let out = "";
		function Probe() {
			out = useLocalePackT(AGENT_I18N, importers, rt)("header.newChat");
			return null;
		}
		act(() => root.render(<Probe />));
		expect(out).toBe("New chat");
		await act(async () => {
			rt.emit("es");
			await Promise.resolve();
		});
		await flush();
		expect(out).toBe("ES:New chat");
	});
});
