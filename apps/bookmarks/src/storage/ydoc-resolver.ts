 ...
 * Journal / Tasks / Code Editor); this wires it to the Bookmarks runtime
 * getter.
 *
 ...
 * `getYDocResolverApi()` returns null. `getBookmarkResolver()` then hands
 * back an in-memory resolver (no transport persistence) so the detail's
 * `<BrainstormEditor>` still mounts on a real Y.Doc seeded from the captured
 * `contentBlocks` — the body just isn't persisted outside the session.
 */
...
/** Resolve the entity→Y.Doc resolver `<BookmarkDetail>` binds through. The
 *  shell-installed resolver (persisted Y.Doc bodies) when available; an
...
export function getBookmarkResolver(): YDocResolver {
