...
import { THEME_EDITOR_I18N } from "./i18n";
...
describe("theme-editor locale packs (12.15)", () => {
	...
	const importers = {
		es: async () => ({
			default: { "app.title": "ES:Theme Editor", "app.moreActions": "ES:More actions" },
		}),
	};
	...
	it("applies the Spanish overlay once the pack resolves", async () => {
		...
		let out = "";
		function Probe() {
			out = useLocalePackT(THEME_EDITOR_I18N, importers, rt)("app.title");
			return null;
		}
		act(() => root.render(<Probe />));
		await flush();
		expect(out).toBe("ES:Theme Editor");
	});
	...
	it("re-derives when the runtime locale changes", async () => {
		...
		let out = "";
		function Probe() {
			out = useLocalePackT(THEME_EDITOR_I18N, importers, rt)("app.moreActions");
			return null;
		}
		act(() => root.render(<Probe />));
