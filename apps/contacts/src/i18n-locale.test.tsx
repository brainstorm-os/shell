...
import { CONTACTS_I18N } from "./i18n";
...
describe("contacts locale packs (12.15)", () => {
	...
	const importers = {
		es: async () => ({
			default: { "app.title": "ES:Contacts", "company.untitled": "ES:Untitled company" },
		}),
	};
	...
	it("applies the Spanish overlay once the pack resolves", async () => {
		...
		let out = "";
		function Probe() {
			out = useLocalePackT(CONTACTS_I18N, importers, rt)("app.title");
			return null;
		}
		act(() => root.render(<Probe />));
		await flush();
		expect(out).toBe("ES:Contacts");
	});
	...
	it("re-derives when the runtime locale changes", async () => {
		...
		let out = "";
		function Probe() {
			out = useLocalePackT(CONTACTS_I18N, importers, rt)("company.untitled");
			return null;
		}
		act(() => root.render(<Probe />));
		expect(out).toBe("Untitled company");
		await act(async () => {
			rt.emit("es");
			await Promise.resolve();
		});
		await flush();
		expect(out).toBe("ES:Untitled company");
	});
});
