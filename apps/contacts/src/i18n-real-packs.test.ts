...
import { CONTACTS_I18N, LOCALE_PACK_IMPORTERS } from "./i18n";
...
function firstDivergentKey(pack: Record<string, string>): string | undefined {
	return Object.keys(pack).find(
		(k) => k in CONTACTS_I18N && pack[k] !== (CONTACTS_I18N as Record<string, string>)[k],
	);
}

describe("contacts real overlay packs (12.15)", () => {
	for (const locale of OVERLAY_LOCALES) {
		it(`loads and overlays the ${locale} pack`, async () => {
			...
			const translate = createT(CONTACTS_I18N, pack ?? {});
			expect(translate(k as keyof typeof CONTACTS_I18N)).toBe(packRec[k]);
			expect(translate(k as keyof typeof CONTACTS_I18N)).not.toBe(
				(CONTACTS_I18N as Record<string, string>)[k],
			);
		});
	}
});
